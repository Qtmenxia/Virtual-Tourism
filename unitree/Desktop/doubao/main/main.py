...
from main.camera import get_image, cv_resize
...
async def generate(url = "rtsp://192.168.144.25:8554/main.264", target_width  = 640, target_height = 360, audio_path = "main/audio"):
    global huawei_token
    global tmp_file
    img = get_image(url)
    ...
    asyncio.sleep(1.)
    scores, label = await predict(tmp_file, huawei_token)
    print(label, scores)
    # if float(scores) < 0.9:
    #     label = "others_0"
    # label = 'tushuguan_0'
