...
# 发送事件
async def send_event(ws: websocket, header: bytes, optional: Union[bytes , None],
                     payload: bytes = None):
...
def read_res_content(res: bytes, offset: int):
    content_size = int.from_bytes(res[offset: offset + 4], byteorder='big')
    offset += 4
...
def read_res_payload(res: bytes, offset: int):
    payload_size = int.from_bytes(res[offset: offset + 4], byteorder='big')
    offset += 4
...
def parser_response(res) -> Response:
    ...
    if header.message_type == FULL_SERVER_RESPONSE or AUDIO_ONLY_RESPONSE:
        ...
        if header.message_type_specific_flags == MsgTypeFlagWithEvent:
            optional.event = int.from_bytes(res[offset:8], byteorder='big')
            offset += 4
...
async def filter_markdown_special_chars(text):
    # 移除 Markdown 标题记号 (1-6个#加空格)
    text = re.sub(r'^#{1,6}\s', '', text, flags=re.MULTILINE)
    
    # 移除加粗记号 (**text** 或 __text__)
    text = re.sub(r'(\*\*|__)(.*?)\1', r'\2', text)
    
    # 移除斜体记号 (*text* 或 _text_)
    text = re.sub(r'(\*|_)(.*?)\1', r'\2', text)
    
    return text

async def gen_audio(text, output_path = 'tts-output.mp3'):
    appId = '1121872599'
    token = 'GN-EmsZ6ejb_UiBLahHG5Jbwmj1kkmez'
    speaker = 'zh_male_jieshuoxiaoming_moon_bigtts'
    # text = await filter_markdown_special_chars(text)
    # print(text)
    await run_demo(appId, token, speaker, text, output_path)

if __name__ == "__main__":
    gen_audio("# *我是小明* _很高兴认识你_")